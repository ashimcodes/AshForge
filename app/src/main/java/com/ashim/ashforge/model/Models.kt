package com.ashim.ashforge.model
...
enum class ProviderKind(
    ...
    // True for providers whose credential is stated by the app itself, not
    // supplied by the person using it (e.g. OpenCode Zen's public free tier).
    // The settings UI hides the API key field for these.
    val noUserKeyRequired: Boolean = false,
) {
    OPENCODE_FREE(
        "OpenCode Free",
        "No account needed \u2014 free models",
        ProviderProtocol.ANTHROPIC_GATEWAY,
        "https://opencode.ai/zen",
        "big-pickle",
        noUserKeyRequired = true,
    ),
    ...
    companion object {
        // OpenCode Zen's documented literal credential for its free-tier routes.
        // Not a secret and not tied to any individual account — see
        // https://opencode.ai/docs/zen — so it is safe to ship in source.
        const val OPENCODE_FREE_SEEDED_KEY = "public"
    }
}
...
data class ChatMessage(
    ...
    // Web pages (relative paths) written during this turn that can be opened
    // with the "Open in Local Server" button.
    val previewablePaths: List<String> = emptyList(),
)
