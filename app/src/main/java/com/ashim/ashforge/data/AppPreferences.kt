package com.ashim.ashforge.data
...
class AppPreferences(private val context: Context) {
    ...
    fun loadProvider(vault: ApiKeyVault): ProviderProfile {
        // Fresh installs default to OpenCode Free so the app works with zero setup.
        val kind = runCatching { ProviderKind.valueOf(preferences.getString("provider_kind", null).orEmpty()) }
            .getOrDefault(ProviderKind.OPENCODE_FREE)
        return ProviderProfile(
            ...
            hasSecret = kind.noUserKeyRequired || vault.contains(kind.name),
        )
    }
    ...
    fun saveMessages(projectId: String, chatId: String, messages: List<ChatMessage>) {
        ...
        messages.forEach { m ->
            arr.put(JSONObject().apply {
                ...
                put("previewablePaths", JSONArray(m.previewablePaths))
        ...
        return runCatching {
            ...
            (0 until arr.length()).map { i ->
                ...
                ChatMessage(
                    ...
                    previewablePaths = obj.optJSONArray("previewablePaths")?.let { paths ->
                        (0 until paths.length()).mapNotNull { index -> paths.optString(index).takeIf { it.isNotBlank() } }
                    }.orEmpty(),
                        ...
                        (0 until workItems.length()).mapNotNull { index ->
                            runCatching {
                                workItems.getJSONObject(index).let { item ->
                                    com.ashim.ashforge.model.ActivityItem(
