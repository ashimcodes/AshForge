package com.ashim.ashforge.network

import com.ashim.ashforge.model.ProviderProtocol
                ...
                when (val item = array.opt(index)) {
                    ...
                    is JSONObject -> {
                        ...
                        if (id.isNotBlank()) {
                            ...
                            // OpenCode Zen (and similar gateways) report price under "cost" with
                            // "input"/"output" fields instead of OpenRouter's "pricing" object,
                            // and free model IDs there end in "-free" rather than ":free".
                            val cost = item.optJSONObject("cost")
                            val free = id.endsWith(":free", ignoreCase = true) ||
                                ...
                                cost?.let {
                                    listOf("input", "output").all { field ->
                                        it.optDouble(field, -1.0).let { v -> v <= 0.0 && v != -1.0 }
                                    }
                                } == true
