...
import com.ashim.ashforge.MainActivity
import com.ashim.ashforge.R
...
class RuntimeExecutionService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent?.action ?: ACTION_START) {
            ...
            ACTION_COMPLETE -> finishTask(
                ...
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Ash Forge finished working in $projectName.",
                failed = false,
            )
            ACTION_FAILED -> finishTask(
                title = "Task needs attention",
                detail = intent?.getStringExtra(EXTRA_DETAIL) ?: "Ash Forge could not finish the task.",
                failed = true,
            )
            ACTION_CANCELLED -> {
                taskRunning = false
    ...
    private fun runningNotification(detail: String, includeStop: Boolean): android.app.Notification {
        val builder = NotificationCompat.Builder(this, RUNNING_CHANNEL_ID)
            ...
            .setContentTitle("Ash Forge is working")
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = getSystemService(PowerManager::class.java)
            .newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "com.ashim.ashforge:active-coding-task")
    ...
    companion object {
        const val ACTION_START = "com.ashim.ashforge.START_RUNTIME"
        const val ACTION_STOP = "com.ashim.ashforge.STOP_RUNTIME"
        const val ACTION_PROGRESS = "com.ashim.ashforge.PROGRESS_RUNTIME"
        const val ACTION_COMPLETE = "com.ashim.ashforge.COMPLETE_RUNTIME"
        const val ACTION_FAILED = "com.ashim.ashforge.FAIL_RUNTIME"
        const val ACTION_CANCELLED = "com.ashim.ashforge.CANCEL_RUNTIME"
        ...
        fun ensureNotificationChannels(context: android.content.Context) {
            ...
            manager.createNotificationChannel(
                NotificationChannel(RUNNING_CHANNEL_ID, "Running coding tasks", NotificationManager.IMPORTANCE_LOW).apply {
                    description = "Shows progress while Ash Forge is working in the background"
                },
            )
            manager.createNotificationChannel(
