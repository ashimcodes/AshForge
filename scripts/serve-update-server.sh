...
# ash-forge-update.json) from a local HTTP directory so the in-app
...
#
# Paste BASE_URL + /ash-forge-update.json into the debug app's
...
variant="debug"
apk_name="ash-forge-${variant}.apk"
...
apk_src="$project_dir/app/build/outputs/apk/$variant/app-${variant}.apk"
if [[ ! -f "$apk_src" ]]; then
...
cat > "$serve_dir/ash-forge-update.json" <<JSON
{
...
Files:
  - ash-forge-update.json
  - $apk_name  (sha256: $sha)

APK URL baked into the manifest:
  $base_url/$apk_name

In a debug build of Ash Forge, open Settings → Update channel, paste:
  $base_url/ash-forge-update.json
and tap Use & check.
...
echo "==> Manifest ready at $base_url/ash-forge-update.json"
